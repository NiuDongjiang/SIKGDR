...
from model_b import Net
from evaluate import evaluate
from data_e import DrugDataLoader
from utils import *
import warnings
...
def train(args, dataset, graph_data, cv):
    ...
    for iter_idx in range(1, args.train_max_iter):
        ...
        pred_ratings, drug_out, drug_sim_out, dis_out, dis_sim_out = model(train_enc_graph, train_dec_graph,drug_graph, drug_sim_feat, drug_feat,dis_graph, dis_sim_feat, dis_feat,subgraphs,Two_Stage)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='SIKGDR')
    ...
    parser.add_argument('--train_valid_interval', type=int, default=1000)
    ...
    parser.add_argument('--a_n', type=int, default=10)
    ...
    for times in range(10):
        ...
        args.save_dir = os.path.join("/cadd_data/data/dj/case", args.save_dir)
        if not os.path.isdir(args.save_dir):
            os.makedirs(args.save_dir)

        dataset = DrugDataLoader(args.data_name, args.device, args.num_sub, args.a_n,
        ...
        recalls += recall_list
    print("mean times auc{:4f} ".format(np.mean(aucs)), "mean times auc std{:4f} ".format(np.std(aucs)),
          "mean times aupr{:4f} ".format(np.mean(auprs)), "mean times aupr std{:4f} ".format(np.std(auprs))
          , "mean times f1{:4f} ".format(np.mean(f1s)), "mean times f1 std{:4f} ".format(np.std(f1s)),
          "mean times precision{:4f} ".format(np.mean(precisions)), "mean times precision std{:4f} ".format(np.std(precisions)),
          "mean times recall{:4f} ".format(np.mean(recalls)), "mean times recall std{:4f} ".format(np.std(recalls)))
    print("aucs", aucs)
    print("auprs", auprs)
    print("f1s", f1s)
