...
class GCNLayer(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        #support_strong = th.mm(x, self.weight_strong)
        #support_weak = th.mm(x, self.weight_weak)

        #output_strong = th.spmm(strong_adj, support_strong)
        #output_weak = th.spmm(weak_adj, support_weak)
...
class GCMCLayer(nn.Module):
    ...
    def __init__(self, rating_vals,  # [0, 1]
        ...
        # Add inferred edges from drug-protein-disease associations
        subConv['inferred'] = GCMCGraphConv(user_in_units, msg_units, weight=True, device=device,
                                                    dropout_rate=dropout_rate)
        subConv['rev-inferred'] = GCMCGraphConv(movie_in_units, msg_units, weight=True, device=device,
                                                        dropout_rate=dropout_rate)
    ...
    def forward(self, graph, drug_feat=None, dis_feat=None, Two_Stage=False):
        ...
        # Inferred edge arguments
        mod_args['inferred'] = (None, Two_Stage)
        mod_args['rev-inferred'] = (None, Two_Stage)
...
class Net(nn.Module):
    ...
    def forward(self, enc_graph, dec_graph,
        ...
        return pred_ratings, drug_out_combined, drug_sim_out, dis_out_combined, dis_sim_out
