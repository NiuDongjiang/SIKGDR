...
def train(args, dataset, graph_data, cv):
    ...
    for iter_idx in range(1, args.train_max_iter):
        model.train()
        Two_Stage = False
        pred_ratings, drug_out, drug_sim_out, dis_out, dis_sim_out = model(train_enc_graph, train_dec_graph, drug_graph,
                                                                           drug_sim_feat, drug_feat, dis_graph,
                                                                           dis_sim_feat, dis_feat, subgraphs, Two_Stage)
        ...
        auroc, aupr, f1, precision, recall, y_true, y_score = evaluate2(args, model, graph_data,
...
if __name__ == '__main__':
    ...
    dataset = DrugNovoLoader(args.data_name, args.device, args.num_sub, args.a_n,
                                symm=args.gcn_agg_norm_symm,
                                k=args.num_neighbor)
    #print("col_idx:%d" % len(dataset.col_idx))
    print("row_idx:%d" % len(dataset.row_idx))
    print("Loading dataset finished ...\n")

    args.save_dir = args.data_name
    args.save_dir = os.path.join("cold_start", args.save_dir)
    if not os.path.isdir(args.save_dir):
        os.makedirs(args.save_dir)

    auc_list, aupr_list = [], []

    for cv in range(0, len(dataset.row_idx)):
        args.save_id = cv + 1
        print("===============" + str(cv + 1) + "=================")
