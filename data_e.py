...
_paths = {
    ...
    'Kdataset': '/nfs_home/dongjiang/model/HGCL-DR-main/raw_data/drug_data/Kdataset',
...
class DrugNovoLoader(object):
    def __init__(self,
        ...
        self.drug_protein_df = pd.read_csv(
            os.path.join(
                f'/nfs_home/dongjiang/model/spider-net/raw_data/drug_data/{self._name}/DrugProteinAssociationNumber.csv'),
            index_col=0)
        self.protein_disease_df = pd.read_csv(
            os.path.join(
                f'/nfs_home/dongjiang/model/spider-net/raw_data/drug_data/{self._name}/ProteinDiseaseAssociationNumber.csv'),
            index_col=0)
        self.inferred_drug_disease_edges = self._generate_inferred_edges(self.drug_protein_df, self.protein_disease_df,
                                                                         self.association_matrix, self.a_n)
    ...
    def _generate_inferred_edges(self, drug_protein_df, protein_disease_df, known_matrix, a_n):
        drug_protein_df = drug_protein_df.reset_index()
        protein_disease_df = protein_disease_df.reset_index()

        drug_protein_df.columns = ['drug', 'protein']
        protein_disease_df.columns = ['disease', 'protein']

        drug_to_protein = drug_protein_df.groupby('drug')['protein'].apply(set).to_dict()
        protein_to_disease = protein_disease_df.groupby('protein')['disease'].apply(set).to_dict()

        inferred_edges = set()

        for drug, proteins in drug_to_protein.items():
            disease_counter = dict()  # disease_id -> # of bridging proteins

            for protein in proteins:
                if protein in protein_to_disease:
                    for disease in protein_to_disease[protein]:
                        disease_id = int(disease)
                        disease_counter[disease_id] = disease_counter.get(disease_id, 0) + 1

            for disease_id, count in disease_counter.items():
                drug_id = int(drug)
                if count >= a_n and known_matrix[drug_id, disease_id] == 0:
                    inferred_edges.add((drug_id, disease_id))

        return list(inferred_edges)
    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, add_support=False):
        ...
        if hasattr(self, 'inferred_drug_disease_edges'):
            inferred_drugs = [pair[0] for pair in self.inferred_drug_disease_edges]
            inferred_diseases = [pair[1] for pair in self.inferred_drug_disease_edges]
            data_dict.update({
                ('drug', 'inferred', 'disease'): (inferred_drugs, inferred_diseases),
                ('disease', 'rev-inferred', 'drug'): (inferred_diseases, inferred_drugs)
            })
        ...
        if add_support:
            ...
            # 先构建一个统一的边类型名列表
            etype_names = [to_etype_name(r) for r in self.possible_rel_values]
            if hasattr(self, 'inferred_drug_disease_edges'):
                etype_names.append('inferred')
